...
const svc = __importStar(require("../services/admin.service"));
const cleanupSvc = __importStar(require("../services/cleanup.service"));
    ...
    try {
        ...
        try {
            ...
            if (onboardingData.lat && onboardingData.lng) {
                ...
                console.log(`🗺️ Updated coordinates for request ${id}: lat=${updateData.lat}, lng=${updateData.lng}`);
            }
        ...
        const updated = await prisma_1.prisma.listingRequest.update({
            where: { id },
            data: updateData,
        });
        res.json(updated);
...
r.post('/listing-requests/:id/send-for-signature', async (req, res) => {
    ...
    try {
        ...
        try {
            const { sendOnboardingSignatureEmail } = await Promise.resolve().then(() => __importStar(require('../services/email.service')));
...
r.get('/parkings', async (_req, res, next) => {
    try {
        const parkings = await prisma_1.prisma.parking.findMany({
            include: {
                owner: {
                    select: {
                        id: true,
        ...
        const parkingsWithStats = parkings.map(parking => {
            ...
            console.log(`🏢 Parking ${parking.id} (${parking.address}): ${confirmedBookings.length} confirmed, ${pendingBookings.length} pending, ${totalParkingHours.toFixed(1)}h, ₪${(totalRevenueCents / 100).toFixed(2)}`);
            return {
                ...parking,
...
r.get('/chats', async (req, res, next) => {
    try {
        const { getAllChats } = await Promise.resolve().then(() => __importStar(require('../services/chat.service')));
...
r.post('/chats/:userId/reply', async (req, res, next) => {
    try {
        ...
        const { sendMessage } = await Promise.resolve().then(() => __importStar(require('../services/chat.service')));
...
r.get('/stats/users', async (req, res, next) => {
    try {
        ...
        const totalOwners = await prisma_1.prisma.user.count({ where: { role: 'OWNER' } });
        const totalRegularUsers = await prisma_1.prisma.user.count({ where: { role: 'USER' } });
        // משתמשים עם OAuth
        const googleUsers = await prisma_1.prisma.user.count({ where: { googleId: { not: null } } });
        const facebookUsers = await prisma_1.prisma.user.count({ where: { facebookId: { not: null } } });
        const appleUsers = await prisma_1.prisma.user.count({ where: { appleId: { not: null } } });
        // משתמשים חדשים השבוע
        const weekAgo = new Date();
        weekAgo.setDate(weekAgo.getDate() - 7);
...
r.put('/coupons/:id', async (req, res, next) => {
    try {
        ...
        if (maxUsage !== undefined)
            updateData.maxUsage = maxUsage ? Number(maxUsage) : null;
        if (isActive !== undefined)
            updateData.isActive = Boolean(isActive);
        const coupon = await coupon_service_1.couponService.updateCoupon(id, updateData);
        res.json(coupon);
    }
    catch (error) {
