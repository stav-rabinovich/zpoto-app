 ...
 */
r.get('/active', auth_1.auth, async (req, res, next) => {
    try {
 ...
 */
r.get('/:id', auth_1.auth, async (req, res, next) => {
    try {
 ...
 */
r.patch('/:id/status', auth_1.auth, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
 ...
 */
r.post('/:id/cancel', auth_1.auth, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const current = await svc.getBooking(id);
        if (!current)
 ...
 */
r.get('/pending-approval', auth_1.auth, async (req, res, next) => {
    try {
 ...
 */
r.post('/:id/approve', auth_1.auth, async (req, res, next) => {
    try {
 ...
 */
r.post('/:id/reject', auth_1.auth, async (req, res, next) => {
    try {
        const bookingId = Number(req.params.id);
        const ownerId = req.userId;
 ...
 */
r.get('/:id/time-left', auth_1.auth, async (req, res, next) => {
    try {
        const bookingId = Number(req.params.id);
        if (isNaN(bookingId)) {
            return res.status(400).json({ error: 'Invalid booking ID' });
        }
        const timeLeft = await (0, approval_timeout_service_1.getTimeLeftForApproval)(bookingId);
        if (timeLeft === null) {
            return res.status(404).json({ error: 'Booking not found or not pending approval' });
        }
        res.json({
...
r.get('/find-problematic/:parkingId', async (req, res, next) => {
    try {
        ...
        const problematicBookings = await prisma_1.prisma.booking.findMany({
            where: {
                parkingId,
                startTime: {
...
r.delete('/fix-parking-10', async (req, res, next) => {
    try {
        ...
        const problematicBooking = await prisma_1.prisma.booking.findFirst({
            where: {
                parkingId: 10,
                startTime: {
                    gte: new Date('2025-10-21T00:00:00.000Z'),
