...
r.get('/', auth_1.auth, async (req, res, next) => {
    try {
        ...
        const recentSearches = await prisma_1.prisma.recentSearch.findMany({
            where: { userId },
...
r.post('/', auth_1.auth, async (req, res, next) => {
    try {
        ...
        const existingSearch = await prisma_1.prisma.recentSearch.findFirst({
            where: {
                userId,
                query: searchQuery
            }
        ...
        if (existingSearch) {
            ...
            const updatedSearch = await prisma_1.prisma.recentSearch.update({
        ...
        // יצירת חיפוש חדש
        const recentSearch = await prisma_1.prisma.recentSearch.create({
            data: {
                userId,
                query: searchQuery,
                lat: lat || null,
        ...
        const searchCount = await prisma_1.prisma.recentSearch.count({
            where: { userId }
        });
        if (searchCount > 20) {
            // מחיקת החיפושים הישנים ביותר
            const oldSearches = await prisma_1.prisma.recentSearch.findMany({
                where: { userId },
                orderBy: { createdAt: 'asc' },
                take: searchCount - 20
            });
            if (oldSearches.length > 0) {
                await prisma_1.prisma.recentSearch.deleteMany({
                    where: {
...
r.delete('/:id', auth_1.auth, async (req, res, next) => {
    try {
        ...
        await prisma_1.prisma.recentSearch.delete({
            where: { id: searchId }
        });
...
r.delete('/', auth_1.auth, async (req, res, next) => {
    try {
        const userId = req.userId;
        const result = await prisma_1.prisma.recentSearch.deleteMany({
            where: { userId }
        });
        res.json({
