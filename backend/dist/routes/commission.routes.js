...
router.get('/owner/:ownerId/commissions', async (req, res, next) => {
    try {
        ...
        console.log(`💰 API: Getting commissions for owner ${ownerId} for ${targetMonth}/${targetYear}`);
        const result = await (0, commission_service_1.getOwnerCommissions)(parseInt(ownerId), targetYear, targetMonth);
        res.json({
            success: true,
            data: {
...
router.get('/admin/commissions', async (req, res, next) => {
    try {
        ...
        const result = await (0, commission_service_1.getAllCommissionsForMonth)(targetYear, targetMonth);
        res.json({
            success: true,
            data: {
...
router.post('/admin/process-payout', async (req, res, next) => {
    try {
        ...
        // קבלת עמלות החודש
        const commissions = await (0, commission_service_1.getOwnerCommissions)(ownerId, year, month);
