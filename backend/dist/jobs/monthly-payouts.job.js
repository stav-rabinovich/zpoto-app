...
exports.testMonthlyPayouts = testMonthlyPayouts;
...
async function processMonthlyPayouts() {
    ...
    console.log(`💰 Processing payouts for period: ${lastMonth.toISOString()} - ${endOfLastMonth.toISOString()}`);
    try {
        // קבלת כל העמלות מהחודש שעבר שעדיין לא עובדו
        const unpaidCommissions = await prisma_1.prisma.commission.findMany({
            where: {
                calculatedAt: {
                    gte: lastMonth,
                    lte: endOfLastMonth
        ...
        const ownerCommissions = new Map();
        ...
        for (const [ownerId, commissions] of ownerCommissions) {
            try {
                ...
                console.log(`💰 Creating payout for owner ${ownerId}: ₪${(totalNetOwnerCents / 100).toFixed(2)}`);
                // יצירת רשומת תשלום
                const payout = await prisma_1.prisma.ownerPayout.create({
                    data: {
                        ownerId,
        ...
        console.log(`💰 Monthly payouts processing completed. Created ${payoutsCreated.length} payouts`);
        return {
            success: true,
 ...
 */
async function testMonthlyPayouts() {
