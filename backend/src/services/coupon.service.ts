...
// Types מהPrisma Client
type Coupon = {
  id: number;
  code: string;
  discountType: 'PERCENTAGE' | 'FIXED';
  discountValue: number;
  applyTo: 'SERVICE_FEE' | 'TOTAL_AMOUNT';
  validUntil: Date;
  isActive: boolean;
  usageCount: number;
...
export interface CouponValidationResult {
  ...
  coupon?: Coupon;
  error?: string;
  errorCode?: 'NOT_FOUND' | 'EXPIRED' | 'INACTIVE' | 'MAX_USAGE_REACHED';
}
...
export class CouponService {
   ...
   * עדכון קופון
   */
  async updateCoupon(id: number, data: Partial<CreateCouponData>): Promise<Coupon> {
