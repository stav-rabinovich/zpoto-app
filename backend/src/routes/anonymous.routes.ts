...
r.get('/recent-searches', async (req, res, next) => {
  try {
    ...
    const recentSearches = await prisma.anonymousRecentSearch.findMany({
      where: { deviceId },
...
r.post('/recent-searches', async (req, res, next) => {
  try {
    ...
    const existingSearch = await prisma.anonymousRecentSearch.findFirst({
      where: {
        deviceId,
        query: searchQuery,
      },
    ...
    if (existingSearch) {
      ...
      const updatedSearch = await prisma.anonymousRecentSearch.update({
    ...
    // יצירת חיפוש חדש
    const recentSearch = await prisma.anonymousRecentSearch.create({
      data: {
        deviceId,
        query: searchQuery,
        lat: lat || null,
    ...
    const searchCount = await prisma.anonymousRecentSearch.count({
      where: { deviceId },
    });

    if (searchCount > 20) {
      // מחיקת החיפושים הישנים ביותר
      const oldSearches = await prisma.anonymousRecentSearch.findMany({
        where: { deviceId },
      ...
      if (oldSearches.length > 0) {
        await prisma.anonymousRecentSearch.deleteMany({
          where: {
...
r.delete('/recent-searches/:id', async (req, res, next) => {
  try {
    ...
    await prisma.anonymousRecentSearch.delete({
      where: { id: searchId },
    });
...
r.delete('/recent-searches', async (req, res, next) => {
  try {
    ...
    const result = await prisma.anonymousRecentSearch.deleteMany({
      where: { deviceId },
    });
...
r.get('/saved-places', async (req, res, next) => {
  try {
    ...
    const savedPlaces = await prisma.anonymousSavedPlace.findMany({
      where: { deviceId },
      orderBy: { createdAt: 'desc' },
...
r.post('/saved-places', async (req, res, next) => {
  try {
    ...
    const existingPlace = await prisma.anonymousSavedPlace.findFirst({
      where: {
        deviceId,
        name: name.trim(),
        type,
    ...
    // יצירת מקום שמור חדש
    const savedPlace = await prisma.anonymousSavedPlace.create({
      data: {
        deviceId,
        name: name.trim(),
    ...
    const placesCount = await prisma.anonymousSavedPlace.count({
      where: { deviceId },
    });

    if (placesCount > 20) {
      // מחיקת המקומות הישנים ביותר
      const oldPlaces = await prisma.anonymousSavedPlace.findMany({
        where: { deviceId },
        orderBy: { createdAt: 'asc' },
        take: placesCount - 20,
        select: { id: true },
      ...
      if (oldPlaces.length > 0) {
        await prisma.anonymousSavedPlace.deleteMany({
          where: {
...
r.delete('/saved-places/:id', async (req, res, next) => {
  try {
    ...
    await prisma.anonymousSavedPlace.delete({
      where: { id: placeId },
    });
...
r.delete('/saved-places', async (req, res, next) => {
  try {
    ...
    const result = await prisma.anonymousSavedPlace.deleteMany({
      where: { deviceId },
    });
...
r.get('/favorites', async (req, res, next) => {
  try {
    ...
    const favorites = await prisma.anonymousFavorite.findMany({
      where: { deviceId },
      include: {
        parking: {
          select: {
...
r.post('/favorites', async (req, res, next) => {
  try {
    ...
    if (existingFavorite) {
      ...
      const favoriteWithParking = await prisma.anonymousFavorite.findUnique({
        where: {
          deviceId_parkingId: {
            deviceId,
            parkingId,
...
r.delete('/favorites/:parkingId', async (req, res, next) => {
  try {
    ...
    await prisma.anonymousFavorite.delete({
      where: {
        deviceId_parkingId: {
          deviceId,
          parkingId,
