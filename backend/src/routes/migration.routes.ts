...
r.get('/preview', async (req, res, next) => {
  try {
    ...
    const [favoritesCount, savedPlacesCount, recentSearchesCount] = await Promise.all([
      prisma.anonymousFavorite.count({ where: { deviceId } }),
      prisma.anonymousSavedPlace.count({ where: { deviceId } }),
      prisma.anonymousRecentSearch.count({ where: { deviceId } }),
    ]);

    const totalCount = favoritesCount + savedPlacesCount + recentSearchesCount;

    console.log(
      `📊 Migration preview - Total: ${totalCount}, Favorites: ${favoritesCount}, SavedPlaces: ${savedPlacesCount}, RecentSearches: ${recentSearchesCount}`
    );
 ...
 * Body: { deviceId }
 */
r.post('/anonymous-to-user', auth, async (req: AuthedRequest, res, next) => {
  try {
 ...
 * Body: { deviceId }
 */
r.post('/cleanup-anonymous', auth, async (req: AuthedRequest, res, next) => {
  try {
    const userId = req.userId!;
    const { deviceId } = req.body;
 ...
 */
r.get('/preview', auth, async (req: AuthedRequest, res, next) => {
  try {
    const { deviceId } = req.query;
